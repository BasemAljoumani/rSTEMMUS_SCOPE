function [iter, fluxes, rad, thermal, profiles, soil, RWU, frac, WaterStressFactor, WaterPotential]             ...
    ...
    maxit           = max(iter.maxit, 300); % Octave fix: increase headroom for convergence
    ...
    while CONT                          % while energy balance does not close
        ...
        for i = 1:30
            ...
            %% Octave fix: NaN/Inf guards on heatflux outputs
            lEch(isnan(lEch) | isinf(lEch)) = 0;
            Hch(isnan(Hch)   | isinf(Hch))  = 0;
            lEcu(isnan(lEcu) | isinf(lEcu)) = 0;
            Hcu(isnan(Hcu)   | isinf(Hcu))  = 0;
            lEs(isnan(lEs)   | isinf(lEs))  = 0;
            Hs(isnan(Hs)     | isinf(Hs))   = 0;
        ...
        %% Octave fix: progressive damping to prevent divergence
        if counter == 5
            Wc = 0.6;
        end
        if counter == 10
        ...
        denom_ch = (rhoa * cp) ./ rac + rhoa * lambdah * e_to_q .* sh ./ (rac + rcwh) + 4 * leafbio.emis * sigmaSB * (Tch + 273.15).^3;
        denom_cu = (rhoa * cp) ./ rac + rhoa * lambdau * e_to_q .* su ./ (rac + rcwu) + 4 * leafbio.emis * sigmaSB * (Tcu + 273.15).^3;
        denom_s  = rhoa * cp ./ ras + rhoa * lambdas * e_to_q .* ss / (ras + rss) + 4 * (1 - soil.rs_thermal) * sigmaSB * (Ts + 273.15).^3;

        %% Octave fix: guard denominators against zero/NaN/Inf
        denom_ch(abs(denom_ch) < 1e-10 | isnan(denom_ch) | isinf(denom_ch)) = 1;
        denom_cu(abs(denom_cu) < 1e-10 | isnan(denom_cu) | isinf(denom_cu)) = 1;
        denom_s(abs(denom_s) < 1e-10   | isnan(denom_s)  | isinf(denom_s))  = 1;
        ...
        Tcu(abs(Tcu) > 100) = Ta;
        Ts(abs(Ts) > 100) = Ta;
        %% Octave fix: reset NaN/Inf temperatures to air temp
        Tch(isnan(Tch) | isinf(Tch)) = Ta;
        Tcu(isnan(Tcu(:)) | isinf(Tcu(:))) = Ta;
        Ts(isnan(Ts) | isinf(Ts)) = Ta;
    ...
    if counter >= maxit
        ...
        %% Octave fix: cap fluxes at net radiation when convergence fails
        %  Prevents unbounded latent heat from propagating through the model
        Rnch_safe = Rnhc + Rnhct;
        Rncu_safe = Rnuc + Rnuct;
        lEch = min(lEch, abs(Rnch_safe));
        lEcu = min(lEcu, abs(Rncu_safe));
        lEs  = min(lEs,  abs(Rns));
        lEch = max(lEch, -100);  % physical floor
        lEcu = max(lEcu, -100);
        lEs  = max(lEs,  -100);
    end
