function SoilVariables = calculateHydraulicConductivity(SoilVariables, VanGenuchten, KIT, L_f, ModelSettings)
    ...
    %% Octave fix: nested functions moved to subfunctions below main function end

    for i = 1:ModelSettings.NL
        % slice vectors
        VG = sliceVector_compat(VanGenuchten, ModelSettings.NL, i);
        ...
        for j = 1:ModelSettings.nD
            ...
            SV = sliceVector_compat(SV, lengthX, MN);
            SV = sliceMatrix_compat(SV, lengthX, ModelSettings.nD, MN, j);
...
%% Octave-compatible subfunctions (moved from nested functions)
function [sliced] = sliceVector_compat(structure, lengthX, ix)
    %{
        slice fields of a structure if they are vectors.
        Octave fix: extracted from nested function for full compatibility.
    %}
    sliced = struct();
...
function [sliced] = sliceMatrix_compat(structure, lengthX, lengthY, ix, iy)
    %{
