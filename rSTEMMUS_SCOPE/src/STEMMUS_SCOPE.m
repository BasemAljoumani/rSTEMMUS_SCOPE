...
% Read config path - Octave and Parallel compatible
if exist('CFG', 'var') == 0
    if exist('../runs/path.txt', 'file')
        fid = fopen('../runs/path.txt', 'r');
        CFG = strtrim(fgetl(fid));
        fclose(fid);
    end
end
...
if strcmp(bmiMode, "initialize") || strcmp(runMode, "full")
    ...
    %% 3. file names
    % Cross-platform Excel reading (supports MATLAB Runtime / Mac / Octave).
    % On Octave, xlsread's output shape is unreliable (N auto-trims rows
    % while X stays full-size, causing row-index misalignment in
    % loadParameters). Route Octave unconditionally through readXlsxNative
    % so X is always the full sheet shape, matching MATLAB's xlsread output.
    isOctave_local = exist('OCTAVE_VERSION', 'builtin') ~= 0;
    if isOctave_local
        allCells_fn = io.readXlsxNative([path_input char(parameter_file)], 'filenames');
        % xlsread's 2nd output is text-only: numeric values become ''
        X = allCells_fn;
        for ri = 1:size(X, 1)
            for ci = 1:size(X, 2)
                if isnumeric(X{ri, ci})
                    X{ri, ci} = '';
                end
            end
        end
        ...
        try
            ...
            opts = detectImportOptions([path_input char(parameter_file)], 'Sheet', 'filenames');
            opts.VariableNamingRule = 'preserve';
            T = readtable([path_input char(parameter_file)], opts, 'Sheet', 'filenames');
            X = [T.Properties.VariableNames; table2cell(T)];
            dummy = [];
        end
    end
    j = find(~strcmp(X(:, 2), {''}));
    ...
    %% 4. input data
    % On Octave, route through readXlsxNative for predictable full-size N
    % and X (Octave's xlsread auto-trims N to ~109 rows while X stays at
    % 123, breaking row-index alignment in loadParameters). MATLAB/MCR keep
    % the xlsread path; Mac/MCR without Spreadsheet toolbox falls back to
    % readtable.
    if isOctave_local
        allCells_id = io.readXlsxNative([path_input char(parameter_file)], 'inputdata');
        nRows = size(allCells_id, 1);
        nCols = size(allCells_id, 2);
        N = NaN(nRows, nCols);
        X = cell(nRows, nCols);
        for ri = 1:nRows
            for ci = 1:nCols
        ...
        try
            ...
            opts = detectImportOptions([path_input char(parameter_file)], 'Sheet', 'inputdata');
            opts.VariableNamingRule = 'preserve';
            T = readtable([path_input char(parameter_file)], opts, 'Sheet', 'inputdata');
            N = table2array(T(:, vartype('numeric')));
            X = [T.Properties.VariableNames; table2cell(T)];
        end
    end
    X  = X(9:end, 1);

    % Octave path now always builds full-size N via readXlsxNative; trim
    % to align with the X(9:end) slice. MATLAB xlsread auto-trims N at
    % the source, so no trim needed there.
    if isOctave_local
        N = N(9:end, :);
    end
