function [SoilVariables, HeatMatrices, HeatVariables, HBoundaryFlux, Rn_SOIL, Evap, Trap, r_a_SOIL, Srt, CHK, AVAIL0, RWUs, RWUg, ForcingData] = solveSoilMoistureBalance(SoilVariables, InitialValues, ForcingData, VaporVariables, GasDispersivity, TimeProperties, SoilProperties, ...
    ...
    for i = indxBotm:ModelSettings.NN
        % Guard: reset non-physical hh (NaN, complex, extreme) to previous timestep.
        % Complex hh arises when Van Genuchten formula gets se>1 due to FP precision,
        % producing complex KL_h → complex tridiagonal solution. Must catch here to
        % prevent infinite cascade in next iteration.
        if ~isreal(SoilVariables.hh(i))
            SoilVariables.hh(i) = real(SoilVariables.hh(i));
        end
        if isnan(SoilVariables.hh(i)) || ~isfinite(SoilVariables.hh(i)) || SoilVariables.hh(i) <= -1E12
